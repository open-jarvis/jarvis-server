...
def on_message(c,ud,msg):
	...
	try:
		topic = msg.topic.replace("jarvis/api/", "").replace("/", "__").replace("-", "_")
		data = json.loads(msg.payload.decode())

		res = json.dumps(getattr(Topics, topic)(data)) if "token" in data else json.dumps({"success":False, "error":"token parameter missing"})
		
		if "reply-to" in data:
			logger.s("on_msg", f"successfully ran endpoint '{topic}' and got response '{res}'")
...
def start_server(DIR):
	global ms, logger
	logger = Logger(f"{DIR}/logs/mqtt_server.log", f"{DIR}/logs")
	logger.console_on()
